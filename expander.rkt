                     ...
                     (hash-set! (get-field position-table puzzler-game) id
                                (map (lambda (pos)
                                       (let* ([dest-x (+ (first pos) dx)]
                                         ...
                                         (cond
                                           [(or dest-val-empty? (and has-interaction? (and (string=? (interaction-rule-str (car interaction)) "push") (can-push? dest-val dest-x dest-y dx dy))))
                                            (if (empty? on-exit-action)
                                                (send puzzler-game update-grid-space! "#" (first pos) (second pos))
                                                (send puzzler-game update-grid-space-and-position-table! (interaction-receiver (car on-exit-action)) (first pos) (second pos)))
                                            (send puzzler-game update-grid-space! id (+ (first pos) dx) (- (second pos) dy))
                                            (list (+ (first pos) dx) (- (second pos) dy))]
                                           [(or dest-val-empty? (and has-interaction? (string=? (interaction-rule-str (car interaction)) "grab")))
                                            (if (empty? on-exit-action)
                                                (send puzzler-game update-grid-space! "#" (first pos) (second pos))
                                                (send puzzler-game update-grid-space-and-position-table! (interaction-receiver (car on-exit-action)) (first pos) (second pos)))
                                            (send puzzler-game remove-from-position-table! (interaction-receiver (car interaction)) (+ (first pos) dx) (- (second pos) dy))
                                            (send puzzler-game update-grid-space! id (+ (first pos) dx) (- (second pos) dy))
                                            (list (+ (first pos) dx) (- (second pos) dy))]
                                           [else
                                            (list (first pos) (second pos))])))
