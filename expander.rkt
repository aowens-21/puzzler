...
; These are the initial game state values, needed for the restart button
(define initial-position-table (make-hash))

(define initial-game-grid void)
...
(define (init-map-vector len)
  (set! map-vector (make-vector len (vector "")))
  (set! initial-game-grid (make-vector len (vector ""))))
    ...
    (for-each (lambda (cell x-pos)
                (cond
                  [(not (string=? cell "#"))
                   ...
                   (insert-initial-pos-table cell x-pos inner-map-index)]
                  [else (void)]))
              cells (build-list (length cells) values))
    (vector-set! map-vector inner-map-index (list->vector cells))
    (vector-set! initial-game-grid inner-map-index (list->vector cells))
...
(define (insert-initial-pos-table value x y)
  (if (hash-has-key? initial-position-table value)
      (hash-set! initial-position-table value (append (hash-ref initial-position-table value) (list (list x y))))
      (hash-set! initial-position-table value (list (list x y)))))
...
(define (restart-game)
  ...
  ; it would start chaning the initial grid copy.
